...
| `kb_memory` | 会话记忆条目（R6） | id, type(FACT/PREFERENCE/TODO), topic, text, source(chat/memo/import), ts, embedding | ✅ M5 建表 |
| `file_locations` | 不可解析文件位置记录（R10） | path, archivePath?, name, ext, sizeBytes, mtime, indexedAt | ✅ M4 建表 |
